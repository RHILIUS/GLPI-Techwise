        ...
        foreach ($critical_items as $item) {
            ...
            echo '<br><span style="color: #666; font-size: 0.85rem; background: #e3f2fd; padding: 2px 8px; border-radius: 12px;">' . $item['type'] . '</span>
                    ...
                    <div style="display: flex; flex-direction: column; align-items: flex-end; gap: 8px;">
                        ...
                        <a href="/front/helpdesk.public.php?create_ticket=1&content=Low%20stock%20alert%20for%20' . urlencode($item['name']) . '%20(' . urlencode($item['type']) . ')&subject=Low%20Stock%20-%20' . urlencode($item['name']) . '" target="_blank" style="background: #1976d2; color: white; padding: 6px 16px; border-radius: 15px; font-size: 0.9rem; text-decoration: none; font-weight: 600; transition: background 0.2s;">Submit Ticket</a>
                    </div>
                </div>';
        }
        echo '</div></div>';
        ...
        foreach ($warning_items as $item) {
            ...
            echo '<br><span style="color: #666; font-size: 0.85rem; background: #e3f2fd; padding: 2px 8px; border-radius: 12px;">' . $item['type'] . '</span>
                    ...
                    <div style="display: flex; flex-direction: column; align-items: flex-end; gap: 8px;">
                        ...
                        <a href="/GLPI-Techwise/front/helpdesk.public.php?create_ticket=1&content=Low%20stock%20alert%20for%20' . urlencode($item['name']) . '%20(' . urlencode($item['type']) . '),%20current%20stock:%20' . urlencode($item['stock_count']) . '&subject=Low%20Stock%20-%20' . urlencode($item['name']) . '" target="_blank" style="background: #1976d2; color: white; padding: 6px 16px; border-radius: 15px; font-size: 0.9rem; text-decoration: none; font-weight: 600; transition: background 0.2s;">Submit Ticket</a>
                    </div>
                </div>';
        }
        echo '</div></div>';
